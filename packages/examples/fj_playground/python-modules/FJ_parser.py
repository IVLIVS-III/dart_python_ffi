...
featherweight_java_parser = Lark(r"""
    ...
    expression : "(" expression ")" | cast | variable | method_super_lookup | field_lookup | new_class

    variable : identifier

    method_super_lookup : expression "." right_side

    right_side : "super{}." right_side | identifier "(" expression_list ")"
    ...
    """, start=["char", "identifier", "type", "fjobject", "fjclass",
                "expression", "variable", "field_lookup",
                "method_super_lookup",
...
class TreeToFJ(Transformer):
    ...
    keywords: list[str] = ["Object", "class", "extends", "super", "return",
                           "new"]
    ...
    def check_method_name(self, string: str):
        if "super" in string:
    ...
    def right_side(self, right_side):
        match right_side:
            case [method, parameters]:
                return FJ.MethodSuperLookUpRight((method, parameters))
            case [right_side]:
                return FJ.MethodSuperLookUpRight(right_side)
            case _:
                raise Exception("The right side of a Methodinnvocation must be either a call to super or a direkt method call")

    def method_super_lookup(self, method_super_lookup):
        expression, right_side = method_super_lookup
        return FJ.MethodSuperLookup(expression, right_side)
