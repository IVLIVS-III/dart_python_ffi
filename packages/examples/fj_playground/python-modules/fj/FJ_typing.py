...
def typecheck_expression(expression: FJ.Expression, env: dict[str, FJ.Type], CT: dict[str, FJ.ClassDef], warnings: list[StupidWarning], error_prefix: str) -> FJ.Type:
    match expression:
        ...
        case FJ.MethodSuperLookup(expression, right_side):
            # recursively getting the type of the expression
            type_of_expression = typecheck_expression(expression, env, CT, warnings, f"{error_prefix} in the expression of the methodlookup '.?.'")
            # match the righ_side to see if its a call to super
            match right_side:
                case FJ.MethodSuperLookUpRight((method_name, parameters)):
                    # checks if the method exists
                    match type_of_expression:
                        case FJ.FJObject():
                            raise Exception(f"\n{error_prefix} class 'Object' has no method '{method_name}'")
                ...
                case FJ.MethodSuperLookUpRight(inner_content):
                    return typecheck_methodlookup_right_side(type_of_expression, inner_content, env, CT, warnings, f"{error_prefix} in the right side of the super method call")
                case g:
                    raise Exception(f"\n{error_prefix} the right side of the methodinvocation must be either a tuple or a MethodSuperLookUpRight but ist {str(g)}")
...
def typecheck_methodlookup_right_side(type_of_expression: FJ.Type, content: tuple[str, list[FJ.Expression]] | FJ.MethodSuperLookUpRight, env: dict[str, FJ.Type], CT: dict[str, FJ.ClassDef], warnings: list[StupidWarning], error_prefix: list[str]) -> FJ.Type:
    match content:
        case (method_name, parameters):
            match type_of_expression:
                case FJ.FJObject():
                    raise Exception(f"\n{error_prefix} class 'Object' has no method '{method_name}'")
                case FJ.FJClass(name) if method_name not in CT[name].methods:
        ...
        case FJ.MethodSuperLookUpRight(inner_content):
            ...
            match type_of_expression:
                case FJ.FJObject():
                    raise Exception(f"\n{error_prefix} class 'Object' has no superclass")
                case FJ.FJClass(name):
                    superclass = CT[name].superclass
                case _:
                    raise Exception("CANT GO HERE BUT PYTHON NEEDS THIS FOR TYPE INFERENCE")
            # recursively checking Type with new base class
            return typecheck_methodlookup_right_side(superclass, inner_content, env, CT, warnings, f"{error_prefix} in super")
        case _:
            raise Exception(f"{error_prefix} the right side of the super method call isnt a MethodSuperLokkUpRight nor a tuple")
...
def typecheck_method(method: FJ.MethodDef, class_name: str, env: dict[str, FJ.Type], CT: dict[str, FJ.ClassDef], warnings: list[StupidWarning], errors: list[str]) -> bool:
    ...
    for argument, type_of_argument in method.typed_parameters.items():
        if argument == "this":
            errors.append(f"In class '{class_name}' in method '{method.method_name}' arguments are not allowed to be named 'this'")
            is_okey = False
